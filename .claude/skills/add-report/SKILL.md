...
**Required guarantees:**
- **Never use `@@PROCID` for practice scoping.** Earlier drafts of this skill suggested `WHERE PracticeId = (SELECT TOP 1 PracticeId FROM PZ_User WHERE Id = @@PROCID)` which is wrong: `@@PROCID` is the current procedure's `object_id`, not a user id, so the subquery either matches nothing or matches an unrelated row by coincidence. Delete that pattern wherever you see it.
- **Practice scoping rule.** `DataBaseContext` opens the practice-specific connection by HTTP host header, so SPs that read **only per-practice tables** (`Charge`, `Payment`, `Voucher`, etc. inside the per-practice DB) are already scoped by the connection. In that case a bare `WHERE` is sufficient.
- **Use an explicit `@PracticeId int` parameter** when any of the following is true: (a) the SP reads from `PracticeCentral` or any cross-practice table; (b) the SP joins a per-practice table to a shared one; (c) the SP uses dynamic SQL or temp tables; (d) the SP runs in a deployment model where one DB holds rows from multiple practices (the planned schema-per-practice model in PostgreSQL still treats this as a "shared rows" scenario for any cross-schema query). When in doubt, **pass `@PracticeId` from the caller and filter explicitly** — this matches the API_ARCHITECTURE.md rule that per-practice queries should filter on `LoggedUser.PracticeId`. The cost of an extra parameter is zero; the cost of a missing filter is a cross-tenant data leak.

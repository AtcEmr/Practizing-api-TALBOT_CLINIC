...
- [docs/database/postgres-schema-conversion.md](./postgres-schema-conversion.md) — table-side conversion **worksheet**: per-decision and per-rule status, gated on Phase 0, plus per-table progress. Update as decisions land.
- [docs/database/postgres-sp-conversion.md](./postgres-sp-conversion.md) — SP-side conversion **worksheet**: per-procedure migration tracker for all 131 user SPs (category, active dispatch, risk signals, recommended disposition, status). Parallel to the table worksheet.
